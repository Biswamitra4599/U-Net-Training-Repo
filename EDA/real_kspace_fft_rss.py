#!/usr/bin/env python3
"""
DDP Real-valued U-Net training on FFT of RSS images.
...
Launch: torchrun --nproc_per_node=2 train_unet_fft_rss_ddp.py
"""
...
# ==========================================
# 1. MODEL ARCHITECTURE (channels: 48→96→184→360→720, 2ch in/out)
# ==========================================
...
class UNET_encoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.first_double_conv = doubleConv2D(2, 48)       # 2-channel input
...
class UNET_decoder(nn.Module):
    def __init__(self):
        ...
        self.double_conv1 = doubleConv2D(720, 360)       # 360 + 360

        self.tcv2 = nn.ConvTranspose2d(360, 184, kernel_size=2, stride=2)
        self.double_conv2 = doubleConv2D(368, 184)       # 184 + 184

        self.tcv3 = nn.ConvTranspose2d(184, 96, kernel_size=2, stride=2)
        self.double_conv3 = doubleConv2D(192, 96)        # 96 + 96

        self.tcv4 = nn.ConvTranspose2d(96, 48, kernel_size=2, stride=2)
        self.double_conv4 = doubleConv2D(96, 48)         # 48 + 48
...
class UNET_final(nn.Module):
    def __init__(self):
        ...
        self.one_cross_one_conv = nn.Conv2d(48, 2, kernel_size=1, stride=1)  # 2-channel output
...
# ==========================================
def fft2c(x):
    """Centered FFT2: image → k-space (complex in, complex out)."""
...
def ifft2c(kspace):
    """Centered IFFT2: k-space → image (complex in, complex out)."""
    return torch.fft.fftshift(
        torch.fft.ifft2(
...
def twoch_kspace_to_image(tensor_2ch):
    """(B, 2, H, W) real k-space → (B, 1, H, W) magnitude image."""
    complex_kspace = twoch_to_complex(tensor_2ch)
    complex_image = ifft2c(complex_kspace)
    return torch.abs(complex_image)
...
def prepare_rss_kspace_input(batch, device):
    """Multi-coil masked i-space → RSS → FFT → 2-channel real."""
    x = batch["masked_i_space"].to(device, dtype=torch.float32)
    multicoil = torch.complex(x[..., 0], x[..., 1])              # (B, coils, H, W)
    mag = torch.abs(multicoil)
    rss = torch.sqrt(torch.sum(mag ** 2, dim=1, keepdim=True))   # (B, 1, H, W)
    kspace_complex = fft2c(rss)
    return complex_to_2ch(kspace_complex)                          # (B, 2, H, W)


def prepare_rss_kspace_target(batch, device):
    """Full RSS → FFT → 2-channel real."""
...
def normalize_batch(tensor):
    """Per-sample normalization by element-wise abs max. Returns (normalized, scale)."""
    scale = tensor.abs().amax(dim=(1, 2, 3), keepdim=True)        # (B, 1, 1, 1)
...
class KSpaceImageHybridLoss(nn.Module):
    """
    L1 on k-space (input-normalized, no extra division) +
    SSIM on IFFT magnitude images (re-normalized by target image max).
    """
    ...
    def forward(self, pred_kspace_2ch, target_kspace_2ch):
        # ── L1 directly on input-normalized k-space ──
        kspace_l1 = self.l1(pred_kspace_2ch, target_kspace_2ch)

        # ── SSIM on image domain, re-normalized by target image max ──
        pred_img = twoch_kspace_to_image(pred_kspace_2ch)
        target_img = twoch_kspace_to_image(target_kspace_2ch)
...
def main():
    ...
    saved_model_path = "/home/biswamitra/health/knee_data/EDA/saved_model/unet_kspace_rss_fft_model_ddp.pth"
    saved_model_dir = os.path.dirname(saved_model_path)
    metrics_path = os.path.join(saved_model_dir, "training_kspace_metrics_rss_fft_ddp.json")
    ...
    if rank == 0:
        ...
        print("Training: FFT(RSS) U-Net with image SSIM (DDP, input-normalized)")
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for batch in train_iter:
                input_kspace = prepare_rss_kspace_input(batch, device)    # (B, 2, H, W)
                target_kspace = prepare_rss_kspace_target(batch, device)  # (B, 2, H, W)
                ...
                # ── PSNR: denormalize → image domain → normalize by RSS max ──
                with torch.no_grad():
