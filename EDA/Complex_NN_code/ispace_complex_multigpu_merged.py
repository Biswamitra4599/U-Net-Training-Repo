...
# ==========================================
# Only change: 15 → 1 in encoder input and final 1x1 conv output
...
class ComplexUNET_encoder(nn.Module):
    def __init__(self):
        super().__init__()
        # ── CHANGED: 15 → 1 input channel (single RSS image) ──
...
def prepare_rss_input(batch, device):
    """
    Compute RSS from masked multi-coil i-space, then wrap as
    a complex tensor (real=RSS, imag=0) with shape (B, 1, H, W).
    """
    x = batch["masked_i_space"].to(device, dtype=torch.float32)
    complex_ispace = torch.complex(x[..., 0], x[..., 1])          # (B, coils, H, W)
    mag = torch.abs(complex_ispace)                                 # magnitude per coil
    rss = torch.sqrt(torch.sum(mag ** 2, dim=1, keepdim=True))     # (B, 1, H, W)
    return torch.complex(rss, torch.zeros_like(rss))                # complex with imag=0


def prepare_rss_target(batch, device):
    """
    Load the ground-truth RSS image and wrap as a complex tensor
    (real=RSS, imag=0) with shape (B, 1, H, W).
    """
    rss = batch["full_rss_combined"].unsqueeze(1).to(device, dtype=torch.float32)  # (B, 1, H, W)
    return torch.complex(rss, torch.zeros_like(rss))


def normalize_complex_batch(tensor):
    """Normalize a complex tensor by its magnitude max. Returns (normalized, scale)."""
...
class SpecialLossFunc(nn.Module):
    """
    Hybrid L1 + SSIM on the magnitude of the complex output.
    Since input is real-as-complex (1 channel), torch.abs() 
    extracts the predicted magnitude image.
    """
...
def main():
    ...
    saved_model_path = "/home/biswamitra/health/knee_data/EDA/saved_model/complex_rss_model_merged.pth"
    saved_model_dir = os.path.dirname(saved_model_path)
    metrics_path = os.path.join(saved_model_dir, "training_metrics_rss_merged.json")
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for batch in train_iter:
                # ── Prepare RSS as complex tensors (B, 1, H, W) ──
                ...
                # ── Normalize by INPUT magnitude max ──
                input_norm, scale = normalize_complex_batch(input_complex)
                target_norm = target_complex / scale
                ...
                # ── PSNR ──
                with torch.no_grad():
                    # Denormalize and take magnitude
            ...
            with torch.no_grad():
                for batch in val_iter:
                    ...
                    # ── PSNR ──
                    pred_rss = torch.abs(pred_norm * scale)
                    rss_max = target_rss_raw.amax(dim=(1, 2, 3), keepdim=True).clamp(min=1e-9)
                    psnr_val = psnr_module(pred_rss / rss_max, target_rss_raw / rss_max)
