#!/usr/bin/env python3
"""
Complex U-Net training using simulated-complex (B,C,H,W,2) representation.
Uses EfficientComplexBatchNorm2d and new real-tensor-based complex layers.

Tensors flow as (B, C, H, W, 2) float32 through the model.
Native complex is used ONLY for FFT operations in data prep and loss.

Launch: torchrun --nproc_per_node=2 complex_kspace_ft_rss_v2.py
"""
...
# from complex_layer_nipun.complex_utils import complex_modulate, complex_demodulate
...
def set_seed(seed: int = 42) -> None:
    ...
    random.seed(seed)
    # Set the seed for NumPy's global RNG
    np.random.seed(seed)
    # Set the seed for PyTorch on all devices (CPU and CUDA)
    torch.manual_seed(seed)
    # Set the seed specifically for CUDA operations (if using a specific CUDA device RNG)
    torch.cuda.manual_seed_all(seed)
    # Set a fixed value for the hash seed
    os.environ["PYTHONHASHSEED"] = str(seed)

    # Configure PyTorch to use deterministic algorithms (can impact performance)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    # Enable deterministic algorithms globally (PyTorch >= 1.8)
    # torch.use_deterministic_algorithms(True) # Optional, can use instead of above cudnn lines

# Usage: Call this function at the very beginning of your script
...
class ComplexDoubleConv2D(nn.Module):
    ...
    def forward(self, x):
        conv_out = self.bn1(self.conv1(x))
        # print(f"Conv output: shape={conv_out.shape}, dtype={conv_out.dtype}, is_complex={conv_out.is_complex()}")
        x = self.relu1(conv_out)
...
def prepare_rss_kspace_input(batch, device):
    ...
    return torch.view_as_real(kspace_complex)             
...
def normalize_sim_complex_batch(tensor):
    ...
    scale = mag.amax(dim=(1, 2, 3), keepdim=True)                 
...
def main():
    ...
    saved_model_path = "/home/biswamitra/health/knee_data/EDA/saved_model/complex_unet_rss_fft_model_v2.pth"
    saved_model_dir = os.path.dirname(saved_model_path)
    metrics_path = os.path.join(saved_model_dir, "training_metrics_complex_rss_fft_v2.json")
    ...
    if rank == 0:
        ...
        print("Training: Complex U-Net (simulated-complex) on FFT(RSS) with DDP")
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for batch in train_iter:
                input_sim = prepare_rss_kspace_input(batch, device)
                target_sim = prepare_rss_kspace_target(batch, device)
                target_rss_raw = (
                ...
                pred_complex = torch.view_as_complex(pred_norm.contiguous())
                target_complex = torch.view_as_complex(target_norm.contiguous())

                loss = loss_func(pred_complex, target_complex)
                ...
                with torch.no_grad():
                    pred_denorm_sim = pred_norm * scale             
                    pred_denorm_complex = torch.view_as_complex(
                        pred_denorm_sim.contiguous()
                    )
                    pred_img = complex_kspace_to_image(pred_denorm_complex)
            ...
            with torch.no_grad():
                for batch in val_iter:
                    ...
                    val_loss = loss_func(pred_complex, target_complex)
